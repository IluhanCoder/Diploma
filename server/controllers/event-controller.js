...
class EventController {
    async addEvent(req, res, next){
        ...
        try{
            const {name, creatorId, desc, genres, date, adress, participants } = req.body
            const creator = await userService.getById(creatorId)
            const creatorName = creator.login
            const event = await EventService.addEvent(name, creatorId, creatorName, desc, genres, date, adress, participants)
