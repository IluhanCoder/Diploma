...
class EventService {
    async addEvent(name, creatorId, creatorName, desc, genres, date, adress, participants, avatar) {
        const event = await EventModel.create({ name, creatorId, creatorName, desc, genres, date, adress, participants } )
        return event
    }
