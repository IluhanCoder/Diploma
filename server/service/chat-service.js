...
class ChatService {
    async newMessage(content, senderId, receiverId) {
        ...
        if(!chat) await chatModel.create({user1Id: senderId, user2Id: receiverId});
    ...
    async getChat(receiverId, senderId){
        ...
        const chat = await chatModel.aggregate([
            {$match: {$or: [{user1Id: convertedReceiverId, user2Id: convertedSenderId}, {user2Id: convertedReceiverId, user1Id: convertedSenderId}]}},
            {$lookup: {
                from: "users",
                localField: "user1Id",
                foreignField: "_id",
    ...
    async getUserChats(userId) {
        ...
        { $match: {$or: [
            {user1Id: convertedUserId},
            {user2Id: convertedUserId}
        ]}},
        {$lookup: {
            from: "users",
            localField: "user1Id",
