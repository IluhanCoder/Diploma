...
export default class EventService {
    ...
    static async fetchUserEvents( creator: string ) : Promise<AxiosResponse<IEvent[]>> {
        return $api.post('/user-events', { creator })
    }

    static async createEvent( name: string, creatorId: string, desc: string, genres: string[], date: Date, adress: string, participants: string[], avatar: File) : Promise<AxiosResponse<IEvent[]>> {
        return $api.post('/events', {name, creatorId, desc, genres, date, adress, participants}).then(res => {
            const data = res.data
